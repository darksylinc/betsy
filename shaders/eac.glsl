...
PotentialSolution storePotentialSolution(const float baseCodeword, const int tableIdx, const float multiplier)
{
  return packUnorm4x8(float4(baseCodeword, float(tableIdx), multiplier, 0.0f) * (1.0f / 255.0f));
}

void loadPotentialSolution(PotentialSolution potentialSolution, out float baseCodeword, out uint tableIdx,
...
float eac_find_best_error(float baseCodeword, float multiplier, const int tableIdx)
{
  ...
  for (int i = 0; i < 16; ++i)
  {
    ...
    {
      const float tryValue = clamp(baseCodeword + kEacModifiers[tableIdx][j] * multiplier, 0.0f, EAC_RANGE);
...
void eac_pack(float baseCodeword, float multiplier, const uint tableIdx)
{
  ...
  // Bits [24; 32)
  outputBytes.x |=
      ((bestIdx[2] & 0x01u) << 31u) | (bestIdx[3] << 28u) | (bestIdx[4] << 25u) | ((bestIdx[5] & 0x04u) << 22u);
  ...
  // Bits [16; 24)
  outputBytes.y |=
      ((bestIdx[10] & 0x01u) << 23u) | (bestIdx[11] << 20u) | (bestIdx[12] << 17u) | ((bestIdx[13] & 0x04u) << 14u);
  // Bits [24; 32)
  outputBytes.y |= ((bestIdx[13] & 0x03u) << 30u) | (bestIdx[14] << 27u) | (bestIdx[15] << 24u);
