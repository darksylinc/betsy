...
const float kPmodeEncoderRG[128] = {
    0,   2,   132, 134, 8,   10,  140, 142, 16,  18,  148, 22,  24,  26,  156, 30,  32,  34,  36,  38,  40, 42,
    44,  46,  48,  50,  52,  54,  56,  58,  60,  62,  64,  66,  68,  70,  72,  74,  76,  78,  80,  82,  84, 86,
    88,  90,  92,  94,  96,  98,  100, 102, 104, 106, 108, 110, 112, 114, 116, 118, 120, 122, 124, 126, 1,  3,
    133, 135, 9,   11,  141, 15,  17,  19,  149, 23,  25,  27,  29,  31,  33,  35,  37,  39,  41,  43,  45, 47,
...
float3 getSrcPixel(uint idx)
{
  const uint2 pixelsToLoadBase =
      ((gl_WorkGroupID.xy << 1u) + uint2(gl_LocalInvocationID.z & 0x01u, gl_LocalInvocationID.z >> 1u)) << 2u;
...
void etc2_planar_mode_write(const float3 cO, const float3 cH, const float3 cV)
{
  ...
  const uint2 dstUV = (gl_WorkGroupID.xy << 1u) + uint2(gl_LocalInvocationID.z & 0x01u, gl_LocalInvocationID.z >> 1u);
...
@param cV [out]
    xz in range [0; 64) y in range [0; 128)
*/
void getCoeffs(const uint2 oIdx, const uint hIdx, const uint vIdx, out float3 cO, out float3 cH, out float3 cV)
{
  float3 cOx, cOy;
...
void main()
{
  ...
  for (uint i = 0u; i < iterations; ++i)
  {
    ...
    if ((thisThreadLocalIdx & mask) == 0u)
    {
      ...
      {
        g_bestCandidates[thisThreadLocalIdx] = float2(nextError, g_bestCandidates[nextThreadLocalIdx].y);
      }
    }
    __sharedOnlyBarrier;
  }
