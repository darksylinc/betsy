...
namespace betsy
{
template <typename T> static inline void write(std::ofstream &fout, const T &v)
{
...
void FormatKTX::save(const char *fullpath, const CpuImage &cpuImage)
{
  ...
  const uint8_t c_KtxFileIdentifier[12] = {0xAB, 0x4B, 0x54, 0x58, 0x20, 0x31, 0x31, 0xBB, 0x0D, 0x0A, 0x1A, 0x0A};
  ...
  {
    const size_t imageSize = CpuImage::getSizeBytes(header.pixelWidth, header.pixelHeight, 1u, 1u, cpuImage.format, 4u);
