...
void EncoderEAC::initResources(const CpuImage &srcImage, const bool rg11)
{
  ...
  m_eacTargetRes[0] = createTexture(TextureParams(getBlockWidth(), getBlockHeight(), gli::FORMAT_RG32_UINT_PACK32,
                                                  "m_eacTargetRes[0]", TextureFlags::Uav));
  if (rg11)
  {
    m_eacTargetRes[1] = createTexture(TextureParams(getBlockWidth(), getBlockHeight(), gli::FORMAT_RG32_UINT_PACK32,
...
void EncoderEAC::startDownload()
{
  ...
  downloadStagingTexture(m_eacTargetRes[1] ? m_stitchedTarget : m_eacTargetRes[0], m_downloadStaging);
}
//-------------------------------------------------------------------------
