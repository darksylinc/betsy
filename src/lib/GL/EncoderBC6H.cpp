...
void EncoderBC6H::initResources(const CpuImage &srcImage)
{
  ...
  m_srcTexture = createTexture(TextureParams(m_width, m_height, srcImage.format, "m_srcTexture"));

  m_compressTargetRes = createTexture(TextureParams(getBlockWidth(), getBlockHeight(), gli::FORMAT_RGBA32_UINT_PACK32,
                                                    "m_compressTargetRes", TextureFlags::Uav));

  m_dstTexture = createTexture(TextureParams(m_width, m_height, gli::FORMAT_RGB_BP_UFLOAT_BLOCK16, "m_dstTexture"));
...
void EncoderBC6H::execute01()
{
  ...
  bindUav(0u, m_compressTargetRes, gli::FORMAT_RGBA32_UINT_PACK32, ResourceAccess::Write);
...
void EncoderBC6H::startDownload()
{
  ...
  if (m_downloadStaging.bufferName)
    destroyStagingTexture(m_downloadStaging);
  m_downloadStaging = createStagingTexture(getBlockWidth(), getBlockHeight(), gli::FORMAT_RG32_UINT_PACK32, false);
...
void EncoderBC6H::downloadTo(CpuImage &outImage)
{
  ...
  outImage.format = gli::FORMAT_RGB_BP_UFLOAT_BLOCK16;
