...
EncoderBC1::EncoderBC1()
    : m_width(0), m_height(0), m_srcTexture(0), m_bc1TargetRes(0), m_bc4TargetRes(0), m_stitchedTarget(0),
      m_dstTexture(0), m_bc1TablesSsbo(0)
{
}
//-------------------------------------------------------------------------
EncoderBC1::~EncoderBC1()
...
void EncoderBC1::initResources(const CpuImage &srcImage, const bool useBC3, const bool bDither)
{
  ...
  m_bc1TargetRes = createTexture(TextureParams(getBlockWidth(), getBlockHeight(), gli::format::FORMAT_RG32_UINT_PACK32,
                                               "m_bc1TargetRes", TextureFlags::Uav));
  m_dstTexture = createTexture(
      TextureParams(m_width, m_height,
                    useBC3 ? gli::format::FORMAT_RGBA_DXT5_UNORM_BLOCK16 : gli::format::FORMAT_RGBA_DXT1_UNORM_BLOCK8,
                    "m_dstTexture"));
  ...
  if (useBC3)
  {
    m_bc4TargetRes = createTexture(TextureParams(getBlockWidth(), getBlockHeight(), gli::FORMAT_RG32_UINT_PACK32,
...
void EncoderBC1::execute01()
{
  ...
  bindUav(0u, m_bc1TargetRes, gli::FORMAT_RG32_UINT_PACK32, ResourceAccess::Write);
  ...
  if (m_bc4TargetRes)
  {
    ...
    bindUav(0u, m_bc4TargetRes, gli::FORMAT_RG32_UINT_PACK32, ResourceAccess::Write);
...
void EncoderBC1::startDownload()
{
  ...
  m_downloadStaging =
      createStagingTexture(getBlockWidth(), getBlockHeight(),
                           m_stitchedTarget ? gli::FORMAT_RGBA32_UINT_PACK32 : gli::FORMAT_RG32_UINT_PACK32, false);
  downloadStagingTexture(m_stitchedTarget ? m_stitchedTarget : m_bc1TargetRes, m_downloadStaging);
}
//-------------------------------------------------------------------------
void EncoderBC1::downloadTo(CpuImage &outImage)
{
  ...
  outImage.format =
      m_stitchedTarget ? gli::format::FORMAT_RGBA_DXT5_UNORM_BLOCK16 : gli::format::FORMAT_RGBA_DXT1_UNORM_BLOCK8;
