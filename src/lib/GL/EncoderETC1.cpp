...
EncoderETC1::EncoderETC1()
    : m_srcTexture(0), m_ditheredTexture(0), m_compressTargetRes(0), m_etc1Error(0), m_eacTargetRes(0),
      m_stitchedTarget(0), m_dstTexture(0), m_etc1TablesSsbo(0)
{
}
//-------------------------------------------------------------------------
EncoderETC1::~EncoderETC1()
...
uint8_t *EncoderETC1::createFilledEtc1Tables()
{
    ...
    for (size_t inten = 0; inten < 8; ++inten)
    {
      for (size_t selector = 0u; selector < 4u; ++selector)
      {
        ...
        for (size_t color = 0u; color < 256u; ++color)
        {
          ...
          asFloat[inverse_table_index * 256u + color] = static_cast<float>(best_packed_c | (best_error << 8u));
        }
      }
    }
  }
  ...
  memcpy(data + startIdx, c_color8_to_etc_block_config_0_255, sizeof(c_color8_to_etc_block_config_0_255));
  memcpy(data + startIdx + sizeof(c_color8_to_etc_block_config_0_255), c_color8_to_etc_block_config_1_to_254,
         sizeof(c_color8_to_etc_block_config_1_to_254));

  return data;
}
//-------------------------------------------------------------------------
bool EncoderETC1::hasAlpha() const
{
  return m_eacTargetRes != 0;
}
//-------------------------------------------------------------------------
void EncoderETC1::initResources(const CpuImage &srcImage, const bool bCompressAlpha, const bool bDither,
                                const bool bForEtc2)
{
  ...
  if (bDither)
  {
    m_ditheredTexture = createTexture(
        TextureParams(m_width, m_height, gli::FORMAT_RGBA8_UNORM_PACK8, "m_ditheredTexture", TextureFlags::Uav));
  ...
  m_compressTargetRes = createTexture(TextureParams(getBlockWidth(), getBlockHeight(), gli::FORMAT_RG32_UINT_PACK32,
                                                    "m_compressTargetRes", TextureFlags::Uav));

  if (bForEtc2)
  {
    m_etc1Error = createTexture(TextureParams(getBlockWidth(), getBlockHeight(), gli::FORMAT_R32_SFLOAT_PACK32,
                                              "m_etc1Error", TextureFlags::Uav));
  }

  m_dstTexture = createTexture(TextureParams(m_width, m_height, gli::FORMAT_RGB_ETC2_UNORM_BLOCK8, "m_dstTexture"));
  ...
  m_compressPso = createComputePso(bForEtc2 ? etc1_with_error_glsl : etc1_glsl);

  if (bCompressAlpha)
  {
    m_eacTargetRes = createTexture(TextureParams(getBlockWidth(), getBlockHeight(), gli::FORMAT_RG32_UINT_PACK32,
...
void EncoderETC1::execute00()
{
  ...
  glDispatchCompute(alignToNextMultiple(m_width, pixelsPerThreadGroup) / pixelsPerThreadGroup,
                    alignToNextMultiple(m_height, pixelsPerThreadGroup) / pixelsPerThreadGroup, 1u);
...
void EncoderETC1::downloadTo(CpuImage &outImage)
{
  ...
  outImage.format = hasAlpha() ? gli::FORMAT_RGBA_ETC2_UNORM_BLOCK8 : gli::FORMAT_RGB_ETC2_UNORM_BLOCK8;
