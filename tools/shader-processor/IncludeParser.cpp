...
bool IncludeParser::parseIncludeFiles(const std::filesystem::path &relativePath)
{
  ...
  while (startPos != std::string::npos)
  {
        ...
        m_source[lineStartPos + 1] != '/' || m_source[lineStartPos + 2] != '/')
    {
      ...
      if (endPos == std::string::npos || (endPos >= eolMarkerPos && eolMarkerPos != std::string::npos))
      {
        printf("Invalid #include syntax near %s",
               m_source.substr(startPos, std::min(startPos + 10u, m_source.size() - startPos)).c_str());
        return false;
      }
...
//-------------------------------------------------------------------------
const char *IncludeParser::getFinalSource() const
{
